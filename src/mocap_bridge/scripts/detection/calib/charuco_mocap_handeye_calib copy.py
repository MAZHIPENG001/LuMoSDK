...
class CharucoMocapHandEye(Node):
    def __init__(self):
        ...
        self.declare_parameter('stationary_wait_timeout_sec', 3.0)
        ...
        self.stationary_wait_timeout_sec = max(
            0.0,
            float(self.get_parameter('stationary_wait_timeout_sec').value),
        )
    ...
    def process_frame(self):
        ...
        if self.show_image:
            ...
            if self.capture_pending:
                status += '  capture=WAITING (hold still)'
            cv2.putText(
                color_image,
                status,
                (10, 28),
    ...
    def request_stationary_pose(self):
        """Save now if possible, otherwise wait for a clean sliding window."""
        with self.capture_lock:
            if self.capture_pending:
                return False

            saved, retryable, reason = self.try_save_stationary_pose()
            if saved:
                return True
            if not retryable or self.stationary_wait_timeout_sec <= 0.0:
                self.get_logger().warn(reason)
                return False

            self.capture_pending = True
            self.capture_deadline_monotonic = (
                time.monotonic() + self.stationary_wait_timeout_sec
            )

        self.get_logger().info(
            'save requested; hold the camera/rigid body still. Waiting up to '
            f'{self.stationary_wait_timeout_sec:g}s for a clean '
            f'{self.averaging_window_sec:g}s window.'
        )
        return False

    def process_pending_capture(self):
        """Retry a requested save as new camera/mocap pairs arrive."""
        ...
        try:
            ...
            timed_out = (
                self.capture_deadline_monotonic is not None
                and time.monotonic() >= self.capture_deadline_monotonic
            )
            if not retryable or timed_out:
                ...
                self.get_logger().warn(
                    ...
                    if not timed_out
                        ...
                        'no stationary window found within '
                        f'{self.stationary_wait_timeout_sec:g}s; not saved: '
                        f'{reason}'
                    )
                )
        finally:
    ...
    def save_stationary_pose(self):
        ...
        with self.capture_lock:
            saved, _, reason = self.try_save_stationary_pose()
        if not saved:
            self.get_logger().warn(reason)
        return saved

    def try_save_stationary_pose(self):
        with self.data_lock:
            if not self.paired_buffer:
                return (
                    False,
                    True,
                    'no synchronized ChArUco/mocap data',
                )
        ...
        if failed:
            return (
                False,
                True,
                'pose is not stationary: ' + '; '.join(failed),
            )
